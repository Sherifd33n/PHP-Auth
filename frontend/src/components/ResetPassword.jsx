...
const ResetPassword = () => {
  ...
  // If there's no token in the URL, someone just typed /reset-password directly
  useEffect(() => {
    if (!token) {
      setMessage({ type: 'error', text: 'Invalid link. Please use the link provided in your email.' });
    }
  }, [token]);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const res = await resetPassword(token, formData.password, formData.repeatPassword);

      if (res.success) {
        setMessage({ type: 'success', text: res.message });
        setFormData({ password: '', repeatPassword: '' });
        
        // Redirect them back to login after showing the success message briefly
        setTimeout(() => {
          navigate('/login');
        }, 3000);
      } else {
  ...
  return (
    <div className="flex items-center justify-center min-h-screen bg-gray-100 p-4">
      <div className="bg-white rounded-2xl shadow-xl w-full max-w-md p-8">
        ...
        {/* Only show the form if we actually have a token to work with */}
        {token && message.type !== 'success' && (
          <form onSubmit={handleSubmit} className="space-y-5">
            ...
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-1" htmlFor="repeatPassword">Confirm New Password</label>
              <input
                id="repeatPassword"
                name="repeatPassword"
                type="password"
